...
attritionByAge <- aggregate(employeeDatRaw$Attrition, by=list(Age=employeeDatRaw$Age), FUN=sum)
sizeByAge <- count(employeeDatRaw,"Age")
attritionRateByAge <- merge(attritionByAge, sizeByAge, by="Age")
attritionRateByAge$Rate <- (attritionRateByAge$x / attritionRateByAge$freq) * 100
names(attritionRateByAge) <- c("Age", "Attrition", "PopulationSize", "AttritionRate")

# display attrition rate by age
knitr::kable(attritionRateByAge,caption = "Attrition Rates by Age", row.names = FALSE, "html") %>%
  kable_styling(bootstrap_options = c("striped","hover", "condensed", "responsive"))
```
